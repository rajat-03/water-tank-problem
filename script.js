function calculateWaterStorage() {
    const renderTables =(heightArray) =>{
      const totalColumns = heightArray.length;
      ...
      leftMaxHeights [0] = heightArray[0];
      for (let i = 1; i < totalColumns; i++) {
        leftMaxHeights [i] = Math.max(leftMaxHeights [i - 1], heightArray[i]);
      }
  
      rightMaxHeights [totalColumns - 1] = heightArray[totalColumns - 1];
      for (let i = totalColumns - 2; i >= 0; i--) {
        rightMaxHeights [i] = Math.max(rightMaxHeights [i + 1], heightArray[i]);
      }
      ...
      for (let i = max; i > 0; i--) {
        ...
        for (let j = 0; j < totalColumns; j++) {
          ...
          // Water (blue)
          else if (heightArray[j] < i && leftMaxHeights [j] >= i && rightMaxHeights [j] >= i) {
